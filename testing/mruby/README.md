...
| Ported module: graphics (real shader-based batched renderer, OpenGL backend; clear/color/rectangle/present + transform stack + render state (blend/scissor/color_mask/line/point/wireframe) + shaders (new_shader/set_shader + Shader type) + canvas/render targets (new_canvas/set_canvas) + stencil/depth state + SpriteBatch + TextBatch + new_image/new_quad/draw + new_font/print/printf; Texture, Quad, Font, Shader, SpriteBatch & TextBatch object types) | `src/modules/graphics/wrap_Graphics_mrb.cpp` |
